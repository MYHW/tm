...
var left=$(".left")[0];
var right=$(".right")[0];
var arrColor=["#ff27a6","#e8e8e8","#ffef01","#e8e8e8","#4c2efe","#ff731c"]
var num=0;
function move(type){
	if(type=="r"){
			num++;
...
hover(dingweia,function(){
	...
	animate(tishia,{left:-150},200);
},function(){
	tishia.style.opacity=0;
	tishia.style.display="none";
	// animate(tishia,{opacity:0},200);
	animate(tishia,{left:-200},200);
})
for(var i=0;i<tishi.length;i++){
	...
	hover(dingwei[i],function(){
		...
		// animate(tishi[this.index],{opacity:1},200);
		animate(tishi[this.index],{left:-90},200)
	},function(){
		tishi[this.index].style.display="none";
		tishi[this.index].style.opacity=0;
		// animate(tishi[this.index],{opacity:0},200);
		animate(tishi[this.index],{left:-140},200)
	})
}
//右侧结束